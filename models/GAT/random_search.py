...
# Train
# ---------------------------------------------------------
def train_one_config(data, hidden, layers, dropout, lr, heads, epochs=50, patience=20, device='cuda'):
    try:
        x = data.x.to(device)
...
def random_search(graph_path, n_iter=20):
    ...
    for i in range(1, n_iter + 1):
        ...
        heads = random.choice(heads_options)

        print(f"Iteration {i}/{n_iter} | Config: hidden={hidden}, layers={layers}, "
              f"dropout={dropout}, lr={lr}, heads={heads}")

        val_loss, state = train_one_config(
            data, hidden, layers, dropout, lr, epochs=50, patience=10, device=device, heads=heads
        )
